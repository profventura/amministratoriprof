...
class SettingsController {
  ...
  public function updateTemplate() {
    ...
    $exists = $pdo->query('SELECT COUNT(*) c FROM settings')->fetch()['c'];
    if ((int)$exists === 0) {
      $stmt = $pdo->prepare('INSERT INTO settings (association_name, receipt_sequence_current, receipt_sequence_year, membership_certificate_template_docx_path) VALUES (?,?,?,?)');
      $stmt->execute(['Associazione AP', 0, (int)date('Y'), $rel]);
    } else {
      $stmt = $pdo->prepare('UPDATE settings SET membership_certificate_template_docx_path=?, updated_at=NOW() ORDER BY id DESC LIMIT 1');
      $stmt->execute([$rel]);
    }
